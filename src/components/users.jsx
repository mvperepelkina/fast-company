...
const Users = () => {
    ...
    return (
        <>
        <h1> <span className={`badge bg-${users.length>0 ? "primary" : "danger"} m-3`}>{renderPhrase(users.length)}</span> </h1>
                ...
                {users.map(user => (
                    <tr key={user._id}>
                        ...
                        <td>{user.qualities.map(q => (
                            <span key={q._id} className={`badge bg-${q.color} m-2`}>{q.name}</span>
                            ))}</td>
